          ...
          <div class="col-md-6">
            ...
            <div class="input-group">
              <select @change="getSelectedType" class="form-control-custom form-control" v-if="$route.params.id !== undefined">
                <option v-for="(each, idx) in types" :key="idx" :value="each.id" :selected="routeParams !== undefined && parseInt(routeParams) === parseInt(each.id) ? true : false">{{each.payload_value}}-3</option>
              </select>
              <select v-model="routeParams" class="form-control-custom form-control" v-if="$route.params.id === undefined">
                <option v-for="(each, idx) in types" :key="idx" :value="each.id" >{{each.payload_value}}-2</option>
              </select>
            </div>
          </div>
          <div class="col-md-6">
